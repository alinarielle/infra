...
in {
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services = mapAttrs (path: val: with val; let
      ...
      removeNull = attrs: (filterAttrs (k: v: v != null) attrs);
      linkedRemotes = foldl' 
        (acc: elem: let
          flagsSubstSecrets = flags: mapAttrs 
            (k: v: if v != null then let
          ...
          getConStr = remote:
            ...
            + (concatMapAttrsStringSep
                ...
                (removeNull (flagsSubstSecrets remote.flags))
              )
            + ":";
        ...
        in acc ++ [{
          ...
          flags = (filterAttrs
            ...
            (mapAttrs
              (k: v: if v != null
                then v
                else if hasSuffix "remote" k
                  then getConStr (head acc)
                  else v
              ) 
              (flagsSubstSecrets elem.flags)
            )
          );
        }])
        [] 
      ...
      configFile = format.generate 
        "rclone.conf"
        (foldl' 
          (acc: elem: with elem; acc // { ${name} = { inherit type; }; } ) 
          {} 
          linkedRemotes
        );
      mountScript = pkgs.writeShellScriptBin
        ...
        + foldl' 
            (acc: remote: acc
              ...
              + concatMapAttrsStringSep
                " "
                (k: v: ''${removePrefix (remote.type + "-") k}=${v}'')
                (filterAttrs 
                  (k: v: any 
                    (x: hasSuffix x k) 
                    ["type"]
                  ) 
                  (removeNull remote.flags)
                )
              + "\n"
            ) 
            ""
        ...
        + ''
          ${rclone} config show 
          ${rclone} mount ${topRemoteConStr} ${path} --daemon --config=${configFile}
        ''
      );
    in {
