{ lib, pkgs, config, ...}: 
with lib; with builtins; config.l.lib.mkLocalModule ./config.nix "general sway config" {
	...
	"librewolf" "mako" "printing" "security" "theme" "wayland" "zathura"
    ];
	...
	wayland.windowManager.sway = {
	    ...
	    config = rec {
	    terminal = "kitty";
	    colors = let color = config.colorScheme.palette; in { 
		focused = {
