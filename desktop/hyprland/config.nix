...
in {
    config = lib.mkIf config.l.desktop.hyprland.enable {
	...
	home-manager.alina.imports = [
	    inputs.hyprland-nix.homeManagerModules.default
	];
	home-manager.alina.wayland.windowManager.hyprland = let
	    color = config.colorScheme.palette; 
	in {
	    enable = true;
	    reloadConfig = true;
	    ...
	    xwayland.enable = if config.l.meta.hidpi then false else true;
	    config.exec_once = [
...
# <https://wiki.hyprland.org/Configuring/Variables/#general>
		...
		inactive_border_color = "#${color.blue}";
