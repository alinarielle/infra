  ...
  in lib.mkForce (pkgs.linuxPackagesFor (kernel.override {
    ...
    structuredExtraConfig = with lib.kernel; {
      ...
      KFENCE = yes;
			...
			# avoid waking up CPUs for sampling
      ...
      # enable BPF JIT; may expose kernel to spray attacks though
      ...
      CPU_IDLE_GOV_TEO = yes;
			...
			# disable legacy system call interfaces
      UID16 = no;
      SGETMASK_SYSCALL = no;
      SYSFS_SYSCALL = no;
      ...
      # detect hard lockups (loops in kernel mode that take more than 10sec)
      HARDLOCKUP_DETECTOR = yes;
      
      # the same but with 20 seconds
      SOFTLOCKUP_DETECTOR = yes;
  
      # detect interurpt storms outside of the callstack
      SOFTLOCKUP_DETECTOR_INTR_STORM = yes;
    };
  }));
  boot.kernelParams = [
      ...
      boot.kernel.sysctl = {
  	# enable useful shortcuts for avoiding freezes in OOM situations
  	"kernel.sysrq" = 1;
  	...
  	# hide kernel memory addresses / kptrs even for processes with CAP_SYSLOG
