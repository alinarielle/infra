  ...
  config =
    ...
    in
    {
      l.folders.create = lib.mapAttrs (
        ...
        in
        lib.mkIf val.persist ((mkDir "/lib") // (mkDir "/log") // (mkDir "/cache"))
      ) cfg.tasks;

      users.users = mapVals (key: val: {
        ${val.user or key} = {
          group = val.group or key;
          isSystemUser = true;
        };
      ...
      systemd.services = lib.mapVals (
        ...
        in
        {
          "${key}-srv" = {
            ...
            script =
              if ((lib.length val.paths.exec) == 1) && (val.script == null) then
                ''
                          #!/bin/bash
