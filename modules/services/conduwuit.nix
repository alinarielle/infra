	...
	options = {
	  ...
	  global.unix_socket_path = lib.mkOption {
	    ...
	    description = ''
	      ...
	      either by adding your reverse proxy to the 'conduwuit' group or granting world
	  ...
	  global.server_name = lib.mkOption {
            type = str;
	    default = "alina.cx";
            description = ''The server_name is the name of this server. 
	    It is used as a suffix for user # and room ids.'';
          };
          global.port = lib.mkOption {
            type = port;
            default = 6167;
            description = ''The port Conduit will be running on. You need to set up a 
	    reverse proxy in your web server (e.g. apache or nginx), so all requests 
	    to /_matrix on port 443 and 8448 will be forwarded to the Conduit instance 
	    running on this port'';
          };
          global.allow_encryption = lib.mkOption {
            type = bool;
            default = true;
            description = ''Whether new encrypted rooms can be created. 
	    Note: existing rooms will continue to work.'';
          ...
          global.address = lib.mkOption {
            type = str;
            default = "::1";
            description = ''Address to listen on for connections by the 
