...
in
{
  ...
  systemd.services.firefly-iii-cron.serviceConfig.WorkingDirectory = lib.mkForce "/var/lib/firefly";
  systemd.services.firefly-iii-cron.serviceConfig.StateDirectory = lib.mkForce "/var/lib/firefly";
  systemd.services.firefly-iii-setup.serviceConfig.StateDirectory = lib.mkForce "/var/lib/firefly";
  systemd.services.firefly-iii-setup.serviceConfig.WorkingDirectory = lib.mkForce "/var/lib/firefly";
  services.firefly-iii = {
    package = pkgs.firefly-iii.overrideAttrs {
      postInstall = ''
        ...
        mv $out/storage ${dataDir}/storage
    ...
    settings = {
      ...
      DB_PORT = 5432; # 5432 for postgresql
      DB_DATABASE = "firefly";
      DB_USERNAME = "firefly";
      DB_PASSWORD_FILE = config.sops.secrets.FIREFLY_DB_PASSWORD.path;
      # leave empty or omit when not using a socket connection
      #DB_SOCKET = postgres requires the unix socket to be specified as above, not here
      ...
      #
      APP_URL = "https://firefly.alina.dog";
    };
  };
}
