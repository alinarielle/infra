  ...
  config = {
    ...
    home-manager.users.alina.xdg = {
      ...
      desktopEntries = mapAttrs (
        ...
        in
        mkIf val.wrapper.xdgDesktopFile {
    ...
    systemd.services = mapAttrs (
      key: val: with val.unix; {
        ...
        // val.environment;
        serviceConfig = {
          ExecStart = optionalString (val.exec != null) (
            escapeSystemdExecArgs (map (x: if isDerivation x then getExe x else x) val.exec)
          );
          Restart = "on-failure";
          RestartSec = "10s";
          StartLimitBurst = 1;
          ...
          ++ val.unix.ro;
          NoExecPaths = "/";
          ExecPaths = val.unix.exec ++ optionals (isDerivation (head val.exec)) (head val.exec);
